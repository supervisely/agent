...
class TaskTrain(TaskDockerized):
    def __init__(self, *args, **kwargs):
        ...
        self.action_map = {str(sly.EventType.CHECKPOINT): self.upload_model}
    ...
    def init_additional(self):
        ...
        if not self.data_mgr.has_nn_storage():
            raise ValueError(
                "Host agent has local neural networks storage disabled. Training without local storage "
    ...
    def download_step(self):
        ...
        if len(self.info["projects"]) != 1:
            raise ValueError(
                "Config contains {} projects. Training works only with single project.".format(
                    len(self.info["projects"])
                )
            )
